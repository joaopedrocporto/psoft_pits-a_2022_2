...
public class EntregadorControllerTests {
    ...
    class EntregadorVerificacaoTipo {
        ...
        void quandoAlteramosEntregadorComTipoVeiculoVazio() throws Exception {
            ...
            assertAll(
                    () -> assertEquals("Erros de validacao encontrados", resultado.getMessage()),
                    () -> assertTrue(resultado.getErrors().contains("Tipo do veiculo deve ser moto ou carro")),
                    () -> assertTrue(resultado.getErrors().contains("Tipo do veiculo e obrigatorio"))
            );
        }
    ...
    class EntregadorDefinirDisponibilidade {
        ...
        void quandoAlteramosDisponibilidadeParaDisponivel() throws Exception {
            ...
            // Assert
            assertEquals("A disponibilidade não pode ser igual a anterior!", resultado.getMessage());
        }
    ...
    @Test
    @DisplayName("Quando alteramos a disponibilidade de um entregador passando codigo de acesso valido")
    void quandoAlteramosDisponibilidadeDeEntregadorComCodigoAcessoValido() throws Exception {
        // Arrange
        ...
        EntregadorResponseDTO resultado = objectMapper.readValue(responseJsonString, EntregadorResponseDTO.EntregadorResponseDTOBuilder.class).build();
        // Assert
        Entregador entregadorZin = this.entregadorRepository.findById(entregador.getId()).orElseThrow(null);
        assertFalse(entregadorZin.getDisponibilidade());
        assertFalse(resultado.isDisponibilidade());
    }
}
